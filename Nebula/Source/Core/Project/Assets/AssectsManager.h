...
namespace Nebula {
	...
	struct AssetPool {
		...
		std::unordered_map<ID, MetadataPair> Metadata; // (polymorphic type)
		...
		template <typename T>
		std::shared_ptr<T> find(const ID& guid) const {
	...
	struct AssetMetaFile {
		AssetMetaFile(ID guid = ID::INVALID, std::filesystem::path sourcePath = "")
	...
	public:
		...
		ID ImportAsset(const std::filesystem::path& assetpath);
		bool RemoveAsset(ID guid);
	...
	private:
		...
		bool LoadAssetFile(const std::filesystem::path& assetpath, ID guid);

		// Loaders
		bool LoadMesh(const std::filesystem::path& assetpath, ID guid);
		bool LoadTexture(const std::filesystem::path& assetpath, ID guid, const int channels = 4);
	};
